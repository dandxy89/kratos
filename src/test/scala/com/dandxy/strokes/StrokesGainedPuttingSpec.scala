...
class StrokesGainedPuttingSpec extends FlatSpec with Matchers with SimulationTestData {
  ...
  "conversion" should "convert to correct metric" in {
    conversion(Distance(3), Feet) shouldBe Distance(1)
    conversion(Distance(1), Meters) shouldBe Distance(1.094)
    conversion(Distance(1), Yards) shouldBe Distance(1)
  }
  ...
  "getStrokesGained" should "run the db call and " in {
    ...
    runIO(getStrokesGained(dbCalled(OnTheGreen))(testUserInput(parThreeExampleGoodGolfer, ParThree))) shouldBe Right(
      -0.6259999999999999
    )
  }
}
