...
class UserQueryToolSpec extends FlatSpec with Matchers with Eventually with BeforeAndAfterAll {
  ...
  it should "register a user correctly" in {
    service.transactQuery(UserQueryTool.registerUser(reg, pass, testTS)) shouldBe testPlayer
  }

  it should "login a user correctly" in {
    service.transactQuery(UserQueryTool.attemptLogin(testEmail, pass)) shouldBe Some(testPlayer)
  }

  it should "return None when a user doesn't exist" in {
    service.transactQuery(UserQueryTool.attemptLogin(fakeEmail, pass)) shouldBe None
  }
