...
def tourney(course_ID, year):
    ...
    data = tourn["years"][0]["tours"][0]["trns"][
        0
    ]  # ignore extraneous metadata for now, get straight to the meat
    ...
    out = {
        ...
        "Year": [],
        "PlayerName": [],
    }  # initialize dictionary for dumping info into
...
def player(PlayerName, year):
    ...
    for CourseID in course_info(year)[
        "CourseID"
    ]:  # get course IDs for all tournaments in given year
        tourn = requests.get(
            "http://www.pgatour.com/data/R/{}/{}/tournsum.json".format(CourseID, year)
        ).json()
        # load JSON for course and year specified
        data = tourn["years"][0]["tours"][0]["trns"][0]["plrs"]
        # ignore extraneous metadata for now, get straight to the meat
...
def course_info(year):
    ...
    for course in ["%03d" % (x,) for x in range(101)]:
        try:
            tourn = requests.get(
                "http://www.pgatour.com/data/R/{}/{}/tournsum.json".format(course, year)
            ).json()  # load JSON for course and year specified
            data = tourn["years"][0]["tours"][0]["trns"][
                0
            ]  # ignore extraneous metadata for now, get straight to the meat
